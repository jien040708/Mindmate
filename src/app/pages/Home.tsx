...
const MOOD_EMOJIS = ["😭", "😢", "😟", "😐", "🙂", "😊", "😄", "🤗"];
const MOOD_VALUES = [10, 25, 40, 50, 60, 75, 85, 95];
  ...
  return [
    {
      ...
      bg:     scores.td  >= 8  ? "bg-blue-50"    : scores.td  <= -3 ? "bg-purple-50"    : "bg-green-50",
    },
    {
      ...
      bg:     scores.ei  >= 7  ? "bg-rose-50"    : scores.ei  <= -1 ? "bg-sky-50"     : "bg-green-50",
    },
    {
      ...
      bg:     scores.pao >= 3  ? "bg-amber-50"   : scores.pao <= -3 ? "bg-teal-50"    : "bg-green-50",
    },
    {
      ...
      bg:     scores.ws  >= 4  ? "bg-orange-50"  : scores.ws  <= -4 ? "bg-red-50"     : "bg-green-50",
    },
  ];
}
...
export default function Home() {
  ...
  const traitCards = selected?.cnipScores ? computeTraitCards(selected.cnipScores, t) : null;
  const oneLiner   = selected?.cnipScores ? computeOneLiner(selected.cnipScores, t) : (selected?.description ?? "");
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 flex-shrink-0 flex flex-col bg-white/50 backdrop-blur-sm border-r border-[#CFF3E4]">
          <div className="px-4 py-3 border-b border-[#CFF3E4]">
            <h2 className="font-bold text-gray-700 text-sm uppercase tracking-wider">{t.counselorList}</h2>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3">
            {personas.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-8 text-gray-400">
                ...
                <p className="text-xs mt-1">{t.noCounselorsDesc}</p>
              </div>
            ) : (
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 py-4 px-2 rounded-2xl border-2 transition-all ${
                        isSelected
                          ? "bg-gradient-to-b from-[#6BCB9A]/20 to-[#355F4B]/10 border-[#6BCB9A] shadow-md"
          ...
          {selected ? (
            <div className="flex-1 flex flex-col overflow-y-auto px-8 py-6 gap-4">
              ...
              <div className="flex items-start justify-between gap-3">
                <h2 className="text-2xl font-bold text-gray-800 leading-tight">{selected.name}</h2>
              ...
              {/* 한 줄 설명 */}
              <p className="text-sm text-gray-500 leading-relaxed">{oneLiner}</p>
              ...
              {traitCards && (
                <div className="grid grid-cols-2 gap-2">
                  {traitCards.map((card) => (
                    <div key={card.dimLabel} className={`${card.bg} rounded-2xl p-3`}>
                      <p className="text-xs text-gray-400 font-medium mb-0.5">{card.dimLabel}</p>
                      <p className={`text-sm font-bold ${card.color}`}>{card.label}</p>
                      <p className="text-xs text-gray-500 leading-tight mt-0.5">{card.desc}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center px-12 gap-4">
              ...
              <p className="text-gray-400 text-lg font-medium">{t.selectCounselor}</p>
              <p className="text-gray-300 text-sm whitespace-pre-line">{t.selectCounselorDesc}</p>
              {personas.length === 0 && (
                <button
                  onClick={() => navigate("/persona")}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-xl font-bold text-gray-800">{t.howAreYou}</h3>
                <p className="text-sm text-gray-400 mt-0.5">{t.moodOptional}</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-4 gap-2 mb-6">
              {MOOD_EMOJIS.map((emoji, i) => {
                ...
                return (
                  <button
                    key={value}
                    onClick={() => setSelectedMood(selectedMood === value ? undefined : value)}
